...
extern int LineThickness=0;
//---
extern int VarLineStyle=0;
extern int VarLineThickness=3;
//---
...
//--- draw the vertical bars that marks the time span
   ...
   SetTimeLine("YesterdayStart","yesterday",idxfirstbarofyesterday,RoyalBlue,level-4*Point);
   SetTimeLine("YesterdayEnd","today",idxfirstbaroftoday,RoyalBlue,level-4*Point);
//---
   if(DebugLogger)
...
//---- High/Low, Open
   if(ShowHighLowOpen)
     {
      SetLevel("Y\'s High",yesterday_high,MediumBlue,VarLineStyle,VarLineThickness,startofyesterday);
      SetLevel("T\'s Open",today_open,MediumBlue,LineStyle,LineThickness,startofday);
      SetLevel("Y\'s Low",yesterday_low,MediumBlue,VarLineStyle,VarLineThickness,startofyesterday);
      //---
      gvname=Symbol()+"yh";
   ...
   if(ShowSweetSpots)
     {
      ...
      ds2= ssp2*Point;
      //---
      SetLevel(DoubleToStr(ds1,Digits),ds1,Maroon,LineStyle,LineThickness,Time[10]);
      SetLevel(DoubleToStr(ds2,Digits),ds2,Maroon,LineStyle,LineThickness,Time[10]);
      //---
      gvname=Symbol()+"ds1";
      gvval=ds1;
      GlobalVariableSet(gvname,gvval);
...
//---- Pivot Lines
   if(ShowPivots==true)
     {
      SetLevel("R1",r1,Red,VarLineStyle,VarLineThickness,startofday);
      SetLevel("R2",r2,Red,VarLineStyle,VarLineThickness,startofday);
      SetLevel("R3",r3,Red,VarLineStyle,VarLineThickness,startofday);
      //---
      SetLevel("Pivot",p,Black,VarLineStyle,VarLineThickness,startofday);
      //---
      SetLevel("S1",s1,Green,VarLineStyle,VarLineThickness,startofday);
      SetLevel("S2",s2,Green,VarLineStyle,VarLineThickness,startofday);
      SetLevel("S3",s3,Green,VarLineStyle,VarLineThickness,startofday);
     }
//---- Fibos of yesterday's range
   if(ShowFibos)
     {
      //--- .618, .5 and .382
      SetLevel("Low - 61.8%", yesterday_low - q*0.618,      DarkGoldenrod, LineStyle, LineThickness, startofday);
      SetLevel("Low - 38.2%", yesterday_low - q*0.382,      DarkOrange, LineStyle, LineThickness, startofday);
      SetLevel("Low + 38.2%", yesterday_low + q*0.382,      DarkOrange, LineStyle, LineThickness, startofday);
      SetLevel("LowHigh 50%", yesterday_low + q*0.5,        Orange, VarLineStyle, LineThickness, startofday);
      SetLevel("High - 38.2%", yesterday_high - q*0.382,    DarkOrange, LineStyle, LineThickness, startofday);
      SetLevel("High + 38.2%", yesterday_high + q*0.382,    DarkOrange, LineStyle, LineThickness, startofday);
      SetLevel("High + 61.8%",yesterday_high +  q*0.618,   DarkGoldenrod, LineStyle, LineThickness, startofday);
      //---
      gvname=Symbol()+"flm618";
   ...
   if(ShowMidPitvot==true)
     {
      ...
      SetLevel("MR3",(r2+r3)/2,Red,LineStyle,LineThickness,startofday);
      SetLevel("MR2",(r1+r2)/2,Red,LineStyle,LineThickness,startofday);
      SetLevel("MR1",(p+r1)/2,Red,LineStyle,LineThickness,startofday);
   ...
   for(int j=0; j<=2*barsperday+1; j++)
     {
      ...
      if(TimeDayOfWeek(timey)==dayofweektofind)
        {
         //--- ignore saturdays (a Sa may happen due to TZ conversion)
