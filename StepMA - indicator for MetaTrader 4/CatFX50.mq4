         ...
         if ((Open[i+1]<EMA50[i+1])&&(Close[i+1]>EMA50[i+1])&&Open[i]>EMA50[i])//cross EMA50
           {
            for(j=confirm_StepMA_Bars; j>=0; j--)
              {
               stepma00=iCustom(NULL,0,"StepMA_Stoch_NK",10,1.1,0,0,i+j);
               stepma01=iCustom(NULL,0,"StepMA_Stoch_NK",10,1.1,0,1,i+j);
               stepma10=iCustom(NULL,0,"StepMA_Stoch_NK",10,1.1,0,0,i+j+1);
               stepma11=iCustom(NULL,0,"StepMA_Stoch_NK",10,1.1,0,1,i+j+1);
               if ((stepma10<stepma11)&&(stepma00>stepma01))//StepMA cross
                 {
                  long1[i]=(Low[i])-iATR(NULL,0,5,i)/2 ;
                  if (i==0&&alert_ON==1)
            ...
            for(j=confirm_StepMA_Bars; j>=0; j--)
              {
               ...
               if ((stepma10>stepma11)&&(stepma00<stepma01))//StepMA cross
                 {
                  short1[i]=High[i]+iATR(NULL,0,5,i)/2;
                  if (i==0&&alert_ON==1)
