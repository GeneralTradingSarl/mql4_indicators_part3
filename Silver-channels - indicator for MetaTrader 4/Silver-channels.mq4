...
//---- стандартные вычисления до 0-го бара
   i=Bars-SSP;
   ...
   while(i>=0)
     {
      ...
      while(k>=i)
        {
         ...
         k--;
        } // все посчитали - записываем значения точки в массив индикатора
      ...
      i--; // двигаем счетчик
     } i=Shift-1; // Считаем точки сдвига - справа от нулевого i=Shift-1;
   while(i>=0)
     {
      high=High[0]; low=Low[0]; k=SSP-Shift-1+i; // каждый следующий уменьшает период k=SSP-Shift+i;
      while(k>=0)
        {
         price=High[k];
         if(high<price) high=price;
